...
row = 0
col = 0
field = 2  # TODO/DONE: Change this to explore data.
...
data_module = HCSDataModule(
    data_path,
    z_window_size=1,
    architecture="UNeXt2_2D",  # TODO# 2D UNeXt2 architecture
...
# </div>
# A1: Nuclei and membrane
# A2: 171 and 45, respectively. The `split_ratio` argument determined it.
...
# </div>
# By performing augmentations (e.g., rotations, crops, scaling, blurring)
# %% [markdown] tags=[]
# <div class="alert alert-info">
#
# ### Task 1.3
...
augmentations = [
    ...
    # ##### DONE ########
    # #######################
    ## TODO: Add Rotation and random noise
    RandWeightedCropd(
...
# <div class="alert alert-warning">
# <h3> Question for Task 1.3 </h3>
# 1. Look at your tensorboard. Can you tell the augmentations were applied to the sample batch? Compare the batch with and without augmentations. <br>
# 2. Are these augmentations good enough? What else would you add?
# </div>
#
# I would also add vertical and horizontal flips. I think those are the only ones missing from the standard transforms (https://docs.pytorch.org/vision/main/auto_examples/transforms/plot_transforms_illustrations.html#sphx-glr-auto-examples-transforms-plot-transforms-illustrations-py). I considered JPEG but its algorightm is based on human perception and that should be irrelevant in this context.
# %% [markdown]
...
phase2fluor_config = dict(
    in_channels=1,  # TODO how many input channels are we feeding? Hint: int,
    out_channels=2,  # TODO how many output channels are we solving for? Hint: int,
...
# </div>
# Yes, the yx goes 256-(down)->128->64->32-(up)->64->128->256.
# The depth goes high and then back down.
...
# %% tags=[]
# ########## DONE ##############
batch_number = 2  # Change this to see different batches of data
# #######################
...
# Hint: Spatial (D,H,W)
gaussian_blur = GaussianFilter(spatial_dims=3, sigma=(0, 1, 2))
# #############################
with torch.inference_mode():
    phase = batch["source"].to(model.device)[:, :, :, y_slice, x_slice]
...
with torch.inference_mode():
    ...
    phase = phase * 1.4  # TODO play here
    # #######################
    pred = model(phase).cpu().numpy()
pred_composite = composite_nuc_mem(pred[0], BOP_BLUE, BOP_ORANGE)
ax[2, 0].imshow(phase[0, 0, 0].cpu().numpy(), cmap="gray", vmin=-15, vmax=15)
